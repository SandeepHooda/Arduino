...
const char* ssid     = "GOGREEN";
const char* password = "F1F1F1F1F1";
// Set web server port number to 80
WiFiServer server(80);
// Variable to store the HTTP request
String header;
// Auxiliar variables to store the current output state
String bellRequest = "off";
...
void setup(){
  pinMode (ringBell, OUTPUT);
  
 digitalWrite (ringBell,LOW);
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            if (header.indexOf("GET /bellRequest/on") >= 0) {
             bellRequest = "on";
            } 
           ...
           client.println("<p>Bell Request "+bellRequest+"</p>");
